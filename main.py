...
class Ui_MainWindow(object):
    ...
    def Ann_function_final(self):
        ...
        if not a:
            self.error(
                '1) Переменной здесь служит ТОЛЬКО x. Не забывайте ввести доплнительные буквенные коэффиценты, ведь происходит апроксимация, а не прямое построение\n\n'
                '2) Задавайте функцию, т.е. вводите только f(x). Пример: k*x+b (y = k*x+b не пойдет)\n\n'
                '3) Точки графика вводятся через пробел, разделителем десятичной части может выступать "," и "."\n\n'
                '4) Количество графиков и количество открытых окон должно в точности совпадать. Иначе будет ошибка\n\n'
                '5) Проследите, что у вас кол-во точек по оси X и по оси Y совпадают\n\n'
                '6) Ошибка возможна, если программа не может посчитать дополнительные галочки, которые вы поставили')
